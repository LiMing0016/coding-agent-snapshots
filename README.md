...
```powershell
docker compose -p stand02-prep build
docker compose -p stand02-prep up -d --no-build --wait
```

浏览器打开 http://127.0.0.1:25520，页面应显示“后端连接正常”。后端通过前端 /api 代理访问，不暴露额外宿主端口。这是开发/评测环境，不作为公网生产部署。

## 环境检查

```powershell
docker compose -p stand02-prep run --rm backend python -m pytest -q
docker compose -p stand02-prep run --rm --no-deps frontend npm test
docker compose -p stand02-prep run --rm --no-deps frontend npm run build
```
...
$env:WEB_PORT='25521'
docker compose -p stand02-ch01-r001-t01-a up -d --no-build --wait
# B: 在另一个 PowerShell 会话与 b 目录中
$env:WEB_PORT='25522'
docker compose -p stand02-ch01-r001-t01-b up -d --no-build --wait
```

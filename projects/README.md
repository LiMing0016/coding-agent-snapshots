...
| Personal English AI 容器准备 | `prep/personalenglishai-container` | 独立根提交，未导入原项目旧历史；详见准备记录 |
...
原项目旧历史曾检测到密钥，不能推送实验副本的完整历史。现已筛选代码树，构造不携带旧历史的独立容器准备快照，并完成 Gitleaks 扫描。[容器准备记录](../tasks/personalenglishai-container-preparation.md)。已有 A/B 分支不覆盖、不改写。
